...
* NOTE The disallowEncryptionWithInsecureAlgorithm option is set to false by default.
* this allows AES-256 CBC encryption which is widely supported even if it is considered less secure.
...
var encryptAssertionXml = (assertionToEncrypt, options) =>
  new Promise((resolve, reject) => {
    ...
    // if the digestAlgorithm is specified in options, replace sha1 with the specified digest algorithm
    if (options.encryptionDigestAlgorithm && options.encryptionDigestAlgorithm !== 'http://www.w3.org/2000/09/xmldsig#sha1') {
      encryptedAssertion = encryptedAssertion.replace('http://www.w3.org/2000/09/xmldsig#sha1',options.encryptionDigestAlgorithm);
    }
...
exports.create = function (options, callback) {
  ...
  options.includeAttributeFriendlyName = (typeof options.includeAttributeFriendlyName !== 'undefined') ? options.includeAttributeFriendlyName : false;
  ...
  if (options.attributes) {
    ...
    Object.keys(options.attributes).forEach(function(prop) {
      ...
      if (options.includeAttributeFriendlyName){
        attributeElement.setAttribute('FriendlyName', prop);
      }
